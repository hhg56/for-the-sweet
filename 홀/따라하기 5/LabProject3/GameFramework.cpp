...
bool GameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	m_hWnd = hMainWnd;
	//Direct3D 디바이스, 명령 큐, 명령 리스트, 스왑체인 등을 생성하는 함수를 호출한다/.
...
void GameFramework::CreateSwapChain()
{
	...
	dxgiSwapChainDesc.OutputWindow = m_hWnd; dxgiSwapChainDesc.SampleDesc.Count = (m_bMsaa4xEnable) ? 4 : 1;
	...
	//alt+enter키의 동작을 비활성화한다
	m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);
	//스왑체인의 현재 후면 버퍼 인덱스를 저장한다.
...
void GameFramework::CreateDirect3DDevice()
{
	...
	//모든 하드웨어 어댑터에 대하여 특성레벨 12.0을 지원하는 하드웨어 디바이스를 생성한다.
	...
	//특성 레벨 12.0을 지원하는 하드웨어를 생성할 수 없으면 WARP 디바이스를 생성한다.
	if (!pd3dAdapter) {
...
void GameFramework::FrameAdvance()
{
	...
	/*현재 렌더 타겟에 대한 렌더링이 끝나기를 기다린다. GPU가 렌더 타겟(버퍼)을 더 이상 사용하지 않으면 렌더 타겟 의 상태는 프리젠트 상태(D3D12_RESOURCE_STATE_PRESENT)로 바뀔 것이다.*/
	...
	/*스왑체인을 프리젠트한다. 프리젠트를 하면 현재 렌더 타겟(후면버퍼)의 내용이 전면버퍼로 옮겨지고 렌더 타겟 인 덱스가 바뀔 것이다.*/
...
//	m_pdxgiSwapChain->Present1(1, 0, &dxgiPresentParameters);
	...
	m_pszBuffer 문자열이 "LapProject ("으로 초기화되었으므로 (m_pszFrameRate+12)에서부터 프레임 레이트를 문자열로 출력 하여 “ FPS)” 문자열과 합친다. 
	...
	case WM_KEYUP:
		switch (wParam) {
			...
			case VK_F9: {
				...
				DXGI_MODE_DESC dxgiTargetParameters; dxgiTargetParameters.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
