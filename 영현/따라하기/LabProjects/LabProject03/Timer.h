...
private:
	...
	__int64 m_nPerformancFrequency;				// 컴퓨터의 Performance Frequency
	
	float m_fFrameTime[MAX_SAMPLE_COUNT];		// 프레임 시간을 누적하기 위한 배열
	ULONG m_nSampleCount;						// 누적된 프레임 횟수

	unsigned long m_nCurrentFrameRate;			// 현재의 프레임 Rate
	unsigned long m_nFramesPerSecond;			// 초당 프레임 수
	float m_fFPSTimeElapsed;					// 프레임 Rate 계산 소요 시간
...
public:
	...
	unsigned long GetFrameRate(LPTSTR lpszString = NULL, int nCharactoers = 0);	// 프레임 레이트를 반환한다.
	float GetTimeElpased();														// 프레임의 평균 경과 시간을 반환한다.
};
