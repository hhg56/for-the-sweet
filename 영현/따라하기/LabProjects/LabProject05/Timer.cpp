...
{
	if (::QueryPerformanceFrequency((LARGE_INTEGER *)&m_nPerformancFrequency))
	{
		m_bHardwareHasPerformanceCounter = TRUE;
		::QueryPerformanceCounter((LARGE_INTEGER *)&m_nLastTime);
		m_fTimeScale = 1.0f / m_nPerformancFrequency;
	}
	else
	{
...
void CGameTimer::Tick(float fLockFPS)
{
	...
	if (fLockFPS > 0.0f)
	{
		// 이함수의 파라미터(fLockFPS)가 0보다 크면 이 시간만큼 호출한 함수를 기다리게 한다.
...
{
	// 현재 프레임 Rate를 문자열로 변환하여 lpszString 버퍼에 쓰고 " FPS"와 결합한다.
	if (lpszString)
	{
...
float CGameTimer::GetTimeElpased()
{
