...
void CGameFramework::OnResiszeBackBuffers()
{
	// 윈도우 클라이언트 영역의 크기가 바뀌면 WM_SIZE 메시지가 발생
	// 윈도우 모드와 전체화면 모드 전환 과정에서도 WM_SIZE가 발생
...
#ifdef __WITH_ONLY_RESIZE_BACKBUFFERS
...
#ifdef _WITH_ONLY_RESIZE_BACKGROUNDS
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	d3dMsaaQualityLevels.SampleCount = 4;		// Msaa 다중 샘플링
	...
	// 펜스와 동기화를 위한 이벤트 객체를 생성한다. 초기값은 FALSE
	// 이벤트가 실행되면 자동적으로 FALSE가 되도록 생성
...
void CGameFramework::CreateDepthStencilView()
{
	...
	D3D12_CLEAR_VALUE d3dClaerValue;
	d3dClaerValue.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	d3dClaerValue.DepthStencil.Depth = 1.0f;
	d3dClaerValue.DepthStencil.Stencil = 0;
	m_pd3dDevice->CreateCommittedResource(&d3dHeapProperties, D3D12_HEAP_FLAG_NONE, &d3dResourceDesc, D3D12_RESOURCE_STATE_DEPTH_WRITE, &d3dClaerValue,
...
{
	if (m_pScene) m_pScene->AnimateObjects(m_GameTimer.GetTimeElpased());
}
...
{
	// 타이머의 시간이 갱신되도록 하고 프레임 Rate를 계산한다.
	...
	// 현재 렌더 타겟에 해당하는 서술자의 CPU 주소(핸들)를 계산한다.

	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle = m_pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	// 깊이-스텐실 서술자의 CPU 주소(핸들)를 계산한다.

	m_pd3dCommandList->OMSetRenderTargets(1, &d3dRtvCPUDescriptorHandle, FALSE, &d3dDsvCPUDescriptorHandle);
	// 렌더 타겟 뷰(서술자)와 깊이-스텐실 뷰(서술자)를 출력-병합 단계(OM)예 연결한다.
	...
	d3dResourceBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	m_pd3dCommandList->ResourceBarrier(1, &d3dResourceBarrier);
	// 현재 렌더 타겟에 대한 렌더링이 끝나기를 기다린다. GPU가 렌더 타겟(버퍼)을 더 이상 사용하지 않으면 렌더 타겟의
	// 상태는 프리젠트 상태(D3D12_RESOURCE_STATE_PRESENT)로 바뀔 것이다.
	...
	*/
	...
	// 스왑 체인을 프리젠트한다.
	// 프리젠트하면 현재 렌더 타겟(후면버퍼)의 내용이 전면버퍼로 옮겨지고 렌더 타겟 인덱스가 바뀔 것이다.

	//m_nSwapChainBufferIndex = m_pdxgiSwapChain->GetCurrentBackBufferIndex();

// Timer
	// 현재의 프레임 Rate를 문자열로 가져와서 주 윈도우의 타이틀로 출력한다.
	// m_pszBuffer 문자열이 "LabProject ("으로 초기화되었으므로 (m_pszFrameRate+12) 에서부터 프레임 Rate를
	// 문자열로 출력하여 " FPS)" 문자열과 합친다.
	// => Title : "LabProject (??? FPS)"
...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		//WM_SIZE : 윈도우가 생성될 때나 윈도우 크기가 변경될 경우 발생하는 메시지
		m_nWndClientWidth = LOWORD(lParam);
		m_nWndClientHeight = HIWORD(lParam);
		// 윈도우 크기가 변경되면 후면버퍼의 크기를 변경한다.
		OnResiszeBackBuffers();
	}
	case WM_LBUTTONDOWN:
	case WM_RBUTTONDOWN:
