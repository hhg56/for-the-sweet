...
CCamera *CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	{
		XMFLOAT3 rright = XMFLOAT3(m_xmf3Right.x, 0.0f, m_xmf3Right.z);
		XMFLOAT3 uup = XMFLOAT3(0.0f, 1.0f, 0.0f);
		XMFLOAT3 llock = XMFLOAT3(m_xmf3Look.x, 0.0f, m_xmf3Look.z);

		m_xmf3Right = Vector3::Normalize(rright);
		m_xmf3Up = Vector3::Normalize(uup);
		m_xmf3Look = Vector3::Normalize(llock);
		...
		XMFLOAT3 kk = XMFLOAT3(0.0f, 0.0f, 1.0f);

		m_fYaw = Vector3::Angle(kk, m_xmf3Look);
...
CAirplanePlayer::CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, int nMeshes) : CPlayer(nMeshes)
{
	...
	XMFLOAT3 position_set = XMFLOAT3(0.0f, 0.0f, -50.0f);
	SetPosition(position_set);
	...
	nMeshes) : CPlayer(nMeshes)
{
	...
	XMFLOAT3 position_sset = XMFLOAT3(pTerrain->GetWidth()*0.5f, fHeight + 1500.0f, pTerrain->GetLength()*0.5f);
	SetPosition(position_sset);
...
CCamera *CTerrainPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case FIRST_PERSON_CAMERA:
		...
		XMFLOAT3 grabitiy_1 = XMFLOAT3(0.0f, -250.0f, 0.0f);
		SetGravity(grabitiy_1);
	...
	case SPACESHIP_CAMERA:
		...
		XMFLOAT3 grabitiy_2 = XMFLOAT3(0.0f, 0.0f, 0.0f);
		SetGravity(grabitiy_2);
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(SPACESHIP_CAMERA, nCurrentCameraMode);
		m_pCamera->SetTimeLag(0.0f);
	...
	case THIRD_PERSON_CAMERA:
		...
		XMFLOAT3 grabitiy_3 = XMFLOAT3(0.0f, -250.0f, 0.0f);
		SetGravity(grabitiy_3);
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(THIRD_PERSON_CAMERA, nCurrentCameraMode);
		m_pCamera->SetTimeLag(0.25f);
...
void CTerrainPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
	...
	if (xmf3CameraPosition.y <= fHeight)
	{
		...
		if (m_pCamera->GetMode() == THIRD_PERSON_CAMERA)
		{
			...
			XMFLOAT3 getp = GetPosition();
			p3rdPersonCamera->SetLookAt(getp);
		}
	}
}