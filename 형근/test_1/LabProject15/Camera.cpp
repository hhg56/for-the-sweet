...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		벡터이다.*/
		XMFLOAT3 a_position = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, a_position);
		//플레이어의 위치를 중심으로 카메라의 위치 벡터(플레이어를 기준으로 한)를 회전한다.
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, (XMMATRIX)xmmtxRotate);	//수정함
	...
	if (m_pPlayer && (y != 0.0f))
	{
		...
		m_xmf3Up = Vector3::TransformNormal(m_xmf3Up, xmmtxRotate);
		m_xmf3Look = Vector3::TransformNormal(m_xmf3Look, xmmtxRotate);
		XMFLOAT3 b_position = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, b_position);
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, (XMMATRIX)xmmtxRotate);
		m_xmf3Position = Vector3::Add(m_xmf3Position, m_pPlayer->GetPosition());
	}
	if (m_pPlayer && (z != 0.0f))
	{
		...
		m_xmf3Up = Vector3::TransformNormal(m_xmf3Up, xmmtxRotate);
		m_xmf3Look = Vector3::TransformNormal(m_xmf3Look, xmmtxRotate);
		XMFLOAT3 c_position = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, c_position);
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, (XMMATRIX)xmmtxRotate);
		m_xmf3Position = Vector3::Add(m_xmf3Position, m_pPlayer->GetPosition());
	}
}
...
void CFirstPersonCamera::Rotate(float x, float y, float z)
{
	...
	if (m_pPlayer && (z != 0.0f))
	{
		...
		XMFLOAT3 d_position = m_pPlayer->GetPosition();
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, d_position);
		//오프셋 벡터 벡터를 회전한다.
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, (XMMATRIX)xmmtxRotate);
		//회전한 카메라의 위치를 월드 좌표계로 표현한다.
		m_xmf3Position = Vector3::Add(m_xmf3Position, d_position);
		//카메라의 로컬 x-축, y-축, z-축을 회전한다.
		m_xmf3Look = Vector3::TransformNormal(m_xmf3Look, xmmtxRotate);
		m_xmf3Up = Vector3::TransformNormal(m_xmf3Up, xmmtxRotate);
		m_xmf3Right = Vector3::TransformNormal(m_xmf3Right, xmmtxRotate);
