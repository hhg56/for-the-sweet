...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, char *pstrFileName)
{
	FILE *pFile = NULL;
	...
	char pstrToken[64] = { '\0' };
	char pstrGameObjectName[128] = { '\0' };

	UINT nReadBytes = 0;
	BYTE nStrLength = 0;
	int nNameLength = 0;

	nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 14, pFile); //"<GameObjects>:"
	nReadBytes = (UINT)::fread(&m_nObjects, sizeof(int), 1, pFile);
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
		nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 13, pFile); //"<GameObject>:"
		nReadBytes = (UINT)::fread(&nNameLength, sizeof(int), 1, pFile);
		nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
		nReadBytes = (UINT)::fread(pstrGameObjectName, sizeof(char), nNameLength, pFile);
		nReadBytes = (UINT)::fread(&pGameObject->m_xmf4x4World, sizeof(float), 16, pFile);

		pstrGameObjectName[nNameLength] = '\0';
		...
		for (int j = 0; j < i; j++)
		{
			if (!strcmp(pstrGameObjectName, m_ppObjects[j]->m_pstrName))
			{
				...
				pGameObject->SetMaterial(m_ppObjects[j]->m_pMaterial->m_nReflection);
				break;
			}
		}
		if (!pMesh)
		{
			strcpy_s(pstrGameObjectName + nNameLength, 64, ".bin");
			pMesh = new CMesh(pd3dDevice, pd3dCommandList, pstrGameObjectName);
			pGameObject->SetMaterial(i % MAX_MATERIALS);
		}
