...
void CMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, char *pstrFileName)
{
	...
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 14, pFile); //"<BoundingBox>:"
	nReadBytes = (UINT)::fread(&m_xmBoundingBox.Center, sizeof(float), 3, pFile);
	nReadBytes = (UINT)::fread(&m_xmBoundingBox.Extents, sizeof(float), 3, pFile);

	nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 11, pFile); //"<Vertices>:"
	...
	nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 10, pFile); //"<Normals>:"
	nReadBytes = (UINT)::fread(&m_nVertices, sizeof(int), 1, pFile);
	m_pxmf3Normals = new XMFLOAT3[m_nVertices];
	nReadBytes = (UINT)::fread(m_pxmf3Normals, sizeof(float), 3 * m_nVertices, pFile);

	nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 16, pFile); //"<TextureCoords>:"
	nReadBytes = (UINT)::fread(&m_nVertices, sizeof(int), 1, pFile);
	m_pxmf2TextureCoords = new XMFLOAT2[m_nVertices];
	nReadBytes = (UINT)::fread(m_pxmf2TextureCoords, sizeof(float), 2 * m_nVertices, pFile);

	nReadBytes = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pFile);
	nReadBytes = (UINT)::fread(pstrToken, sizeof(char), 10, pFile); //"<Indices>:"
